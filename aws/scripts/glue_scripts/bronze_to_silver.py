...
schema           = eval_values(ingestion_params.get('table_schema', None), trgt_tbl, logger, manager, email_on_failure)
has_bdq          = eval_values(ingestion_params.get('has_bdq', None), trgt_tbl, logger, manager, email_on_failure)
header           = eval_values(ingestion_params.get('header', True), trgt_tbl, logger, manager, email_on_failure)
encoding         = ingestion_params.get('encoding', 'UTF-8')
sep              = ingestion_params.get('sep', None)
options_params   = eval_values(ingestion_params.get('options_params', {}), trgt_tbl, logger, manager, email_on_failure)
explode_column   = ingestion_params.get('explode_column', None)
skip_header      = eval_values(ingestion_params.get('skip_header', None), trgt_tbl, logger, manager, email_on_failure)
skip_footer      = eval_values(ingestion_params.get('skip_footer', None), trgt_tbl, logger, manager, email_on_failure)
filter_column    = ingestion_params.get('filter_column', None)
filter_value     = ingestion_params.get('filter_value', None)
positional_column = eval_values(ingestion_params.get('positional_column', []), trgt_tbl, logger, manager, email_on_failure)
